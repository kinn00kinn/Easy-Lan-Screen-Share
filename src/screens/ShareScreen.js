    ...
    <div>
      ...
      {!offerUrl ? (
        <div style={{ textAlign: 'center' }}>
          <button onClick={startSharing} className="button primary-button">画面共有を開始</button>
          <p className="status">{status}</p>
        </div>
      ) : (
        <>
          <div className="step">
            <h2 className="step-title">STEP 1: 視聴者にURLまたはQRコードを共有</h2>
            <p>このURLをコピーするか、QRコードをスキャンしてもらってください。</p>
          ...
          <div className="step">
            <h2 className="step-title">STEP 2: 視聴者から送られたコードを入力</h2>
            <p>視聴者の画面に表示された返信コードをここに貼り付けてください。</p>
            <textarea
              className="code-box"
              placeholder="返信コードをここにペースト..."
              onPaste={e => handleAnswerSubmit(e.clipboardData.getData('text'))}
            />
          </div>
          <p className="status">{status}</p>
        </>
