    ...
    <div>
      ...
      {!answer ? (
         <>
          ...
          {!window.location.hash && <div id="qr-reader" className="scanner-container" />}
         </>
      ) : (
        <div className="step">
          <h2 className="step-title">返信コードを配信者に送ってください</h2>
          <textarea className="code-box" value={answer} readOnly />
          <button onClick={() => navigator.clipboard.writeText(answer)} className="button primary-button" style={{marginTop: '1rem'}}>コードをコピー</button>
        </div>
      )}
    </div>
  );
